...
(function () {
  ...
  // Images come from the data-URI bundle spread/assets.js (window.BOOK_ASSETS,
  // generated by experiments/bundle-assets.py) so the page also works opened
  // from file://: a sibling JPG would taint the canvas and WebGL would refuse
  // the texture (black cover, no photos — exactly what Igor saw locally while
  // the published page was fine). The plain files stay next to it as fallback
  // and for editing; re-run the bundler after replacing one.
  function loadImg(name, cb) {
    var im = new Image();
    im.onload = function () { cb(im); }; im.onerror = function () { cb(null); };
    im.src = (window.BOOK_ASSETS && window.BOOK_ASSETS[name]) || (BASE + 'spread/' + name);
  }
